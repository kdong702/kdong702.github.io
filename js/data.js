...
const projects = [
    ...
    challenges: [
      {
        ...
        detail: [
          "네이버페이와 통신하는 부분, 현대백화점 내부 시스템과 통신하는 부분을 아예 분리된 두 통로로 구성하고 그 사이를 변환 모듈이 이어주는 구조로 설계했습니다. 두 회사의 통신 규격이 데이터 구조부터 암호화 방식, 시간제한까지 전부 달라서, 하나로 억지로 합쳐두면 한쪽을 수정할 때마다 다른 쪽 로직까지 영향을 받을 위험이 있었습니다. 그래서 네이버페이에서 들어온 요청을 해석하고 풀어낸 다음 내부 시스템이 알아듣는 형식으로 변환해 전달하고, 응답이 오면 반대로 다시 변환해 네이버페이 쪽에 돌려주는 방식을 택했습니다.",
          "여기에 더해, 카드 등록은 네이버페이 쪽에서 자체적으로도 처리할 수 있는 구조라 실시간 승인·등록 요청과는 별개로, 현대백화점 내부 시스템에서 카드 발급이 완료되면 그 결과를 네이버 쪽에 알려주는 발급 통지 전문을 HTTPS로 JSON 포맷에 실어 보내야 했습니다. TCP 바이너리 프로토콜과 HTTPS/JSON은 연결 방식(상시 연결 vs 매번 새 연결)부터 데이터 포맷, 인증 방식까지 전혀 달라서, 이 발급 통지 발신 로직은 기존 TCP 파이프라인과 완전히 분리된 별도 클라이언트 모듈로 구현하고 실패 시 재시도까지 독립적으로 관리하도록 만들었습니다."
        ]
      },
      {
        ...
        detail: [
          "재시도 작업이 클러스터의 모든 서버에서 똑같이 실행되다 보니, 그대로 두면 같은 실패 건을 여러 서버가 동시에 처리해 네이버페이 쪽에 중복 통보가 나갈 우려가 있었습니다. 그래서 서버가 켜질 때 자기 번호를 확인해두고, 매분 도는 재시도 작업에서 '지금 몇 분인지'를 서버 대수로 나눈 나머지가 자기 번호와 같은 서버만 실제로 재시도하도록 구성했습니다.",
          "실패 횟수는 DB에 누적해두다가 3번을 넘기면 그 시점에 담당자에게 문자를 보냅니다."
        ]
      },
      {
      ...
      },
      {
        ...
        detail: [
          "네이버페이가 보낸 요청은 현대백화점이 가진 비공개 키로 복호화하되, 암호화가 필요하다고 표시된 항목만 골라 처리했습니다. 반대로 응답을 구성할 때는 매번 새 세션키를 생성해 네이버페이 공개 키로 감싸고, 그 세션키로 응답 항목을 암호화했습니다.",
          "데이터 전체를 일괄 암호화하지 않고 항목 단위로 처리한 이유는, 암호화 대상 항목이 요청 종류마다 달라서 항목별로 표시해두는 방식이 아니면 하나의 공통 로직으로 처리하기 어려웠기 때문입니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          "보안 장비에 저장된 키는 이름(라벨)으로 조회하는데, 실제 등록된 라벨을 확인해보니 지정된 길이(예: 255바이트)만큼 뒤에 Null 문자가 채워진 채로 저장돼 있었습니다. 그래서 애플리케이션에서 짧은 문자열 이름을 그대로 넘겨 조회하면 길이가 다르다는 이유로 정확히 일치하는 라벨을 찾지 못하는 경우가 있었는데, 입력받은 이름 그대로 먼저 한 번 조회해보고 실패하면 남는 자리만큼 Null 패딩을 채운 라벨로 다시 조회하는 폴백(fallback) 로직을 추가해 해결했습니다.",
          "키를 찾은 뒤 실제 인증을 시도하는 단계에서도 비슷한 폴백이 필요했습니다. 카드가 아직 초기 상태인지 이미 발급이 끝난 상태인지에 따라 유효한 키가 달랐는데, 하나의 키만 시도해서 실패하면 바로 인증 실패로 처리했더니 카드 상태를 다시 확인하고 처음부터 재요청해야 해서 시간이 꽤 걸렸습니다. 그래서 후보 키들을 순서대로 담아두고 하나가 맞지 않으면 다음으로 자동 전환되도록, 마지막 키까지 실패했을 때만 인증 실패로 처리하도록 개선해 카드 상태와 무관하게 요청 한 번으로 인증이 끝나도록 만들었습니다.",
          "여기에 더해, 실제 운영에서는 은행이 발급한 키 이름을 써야 하고 로컬·개발에서는 테스트용 이름을 써야 했기 때문에, 이 두 가지 이름도 환경 설정값으로 분리해 배포 환경에 따라 코드 수정 없이 자동으로 전환되도록 만들었습니다."
        ]
      },
      {
    ...
    challenges: [
      {
        ...
        detail: [
          ...
          "한편 보안 장비와 직접 통신하는 부분은 별도 인터페이스 하나로 감싸 두어서, 평소 테스트에서는 이 인터페이스를 가짜 구현으로 바꿔치기해 프로토콜 처리 로직만 실제 장비 없이 빠르게 반복 검증할 수 있게 했습니다. 반대로 실제 물리 보안 장비가 있어야만 의미가 있는 테스트는 평소 테스트 실행에는 포함하지 않고, 필요할 때 환경변수를 켰을 때만 실행되도록 분리해, 장비가 없는 환경에서도 전체 테스트가 항상 문제없이 통과하도록 만들었습니다."
        ]
      },
      {
        ...
        detail: [
          "국내 표준 암호화 방식은 국제 표준에 정의돼 있지 않다 보니 보안 장비 제조사마다 자체적으로 정한 코드값을 쓰는데, 제조사마다 이 값이 서로 달랐습니다. 코드 곳곳에서 제조사를 직접 구분해 처리하면 제조사가 늘 때마다 코드 전체를 수정해야 해서, 제조사별 코드값을 별도 설정값으로 분리해두고 지금 설정된 제조사에 맞는 값을 자동으로 조회해 반환하는 구조로 전환했습니다. 그러면 새 제조사가 추가돼도 설정값 하나만 추가하면 됩니다.",
          "실제 운영 환경은 물리 HSM 장비 2대로 이중화된 구성인데, 장비마다 슬롯에 같은 라벨을 쓰다 보니 라벨 하나만으로는 어느 장비의 슬롯인지 구분할 수 없었습니다. 그래서 슬롯 목록을 순서대로 훑으면서, 관리자 토큰 계열 라벨이 나오면 그걸 '이 장비의 슬롯 목록은 여기까지'라는 경계 마커로 취급하고, 마커를 지날 때마다 접두사를 다음 것으로 전환하도록 만들었습니다. 그렇게 얻은 접두사를 라벨 앞에 붙여 '접두사_라벨' 형태의 키로 슬롯을 관리하니, 장비가 달라도 같은 라벨끼리 서로 덮어쓰지 않고 각각 구분해서 다룰 수 있었습니다."
        ]
      },
      {
        ...
        detail: [
          "폐기된 키를 다루는 방식에는 업계에서 통용되는 원칙(NIST SP 800-57)이 있습니다. 단순히 만료·비활성 처리된 키는 예전에 그 키로 암호화해둔 데이터를 나중에 읽어야 할 수도 있으니 복호화는 허용하고, 다만 그 키로 새로운 데이터를 암호화하는 것만 금지하는 게 기본입니다. 반면 유출(키가 노출된 것으로 확인된) 상태는 복호화조차 신뢰할 수 없다고 보고 암호화·복호화 둘 다 막아야 합니다.",
          "이 규칙을 코드 곳곳에 조건문으로 흩어두면 관리가 어려워지므로, (상태, 동작) 조합별로 허용 여부를 담은 정책 테이블을 따로 두고 이를 검사하는 공통 로직을 만들었습니다.",
          "그리고 관리자 콘솔의 암복호화 테스트 화면과 실제 KMIP 프로토콜로 들어오는 Encrypt/Decrypt 요청이 결과적으로 같은 실행 경로를 타도록 구성해서, 콘솔에서만 검증하고 프로토콜 요청은 그 검증을 빠져나가는 일이 없도록 했습니다. 정책에 걸리면 예외를 던지고, 이 예외는 KMIP 요청을 처리하는 지점에서 표준 규격이 정한 실패 응답으로 변환해 돌려줍니다."
        ]
      }
    ]
  },
  {
    ...
    screenshots: [
      { src: "images/screenshots/cau.png", caption: "학생증 신청 현황 관리 화면 — 소속·발급상태별 조회 (개인정보는 모자이크 처리)" }
    ],
    achievements: [
    ...
    challenges: [
      ...
      },
      {
        ...
        detail: [
          "학생, 교직원, 대학원생, 도서관 이용자, 연구원처럼 소속 유형에 따라 신청 자격, 심사 절차, 발급 방식이 모두 달랐습니다. 여기에 더해 각 유형의 사용자 정보가 하나의 데이터베이스에 모여 있지 않고, 학교 발급 시스템 DB·사회교육원 DB·산학협력단 DB처럼 서로 다른 데이터베이스에 나뉘어 있었습니다. 신청·발급을 처리하는 로직에 유형별로 조건 분기를 계속 추가하면 유형이 늘어날 때마다 기존 코드를 건드려야 해서 유지보수가 어려워질 우려가 있었습니다.",
          "그래서 유형별로 별도의 서비스 구현체를 두고, 공통 인터페이스와 공통 기반 클래스로 반복되는 처리(사용자 조회, 사진 조회 등)를 묶어둔 뒤, 신청·발급을 처리하는 공통 로직에서는 요청에 담긴 사용자 유형 코드를 이름으로 변환해 그에 맞는 구현체를 스프링 컨테이너에서 동적으로 찾아 위임하도록 설계했습니다. 카드(발급) 처리도 같은 방식으로 유형별 구현체를 두고 동일하게 동적으로 위임합니다.",
          "각 유형별 구현체는 자신이 속한 데이터베이스에 연결되도록 미리 구성해뒀기 때문에, 신청·발급을 처리하는 공통 로직 자체는 어떤 데이터베이스를 쓰는지 전혀 알 필요 없이 동일한 방식으로 모든 유형을 처리할 수 있었습니다."
        ]
      },
      {
        ...
        detail: [
          "발급 이력에서 카드 종류별로 가장 최근 '기기변경' 사유의 발급 기록을 확인해, 그 날짜가 오늘이면 재발급을 제한하도록 했습니다.",
          "다만 본인이 아니라 다른 사람(관리자 등)이 분실 신고를 처리해서 넘어간 경우까지 막으면 곤란하니, 그런 경우엔 제한을 해제하는 예외를 별도로 두었습니다. 재직 중인 임직원 카드나 테스트 계정은 이 확인 자체를 앞단에서 건너뛰도록 처리했습니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          "core 서버는 지점의 카드 리더 기기들과 TCP로 상시 연결을 맺고 있는데, 여기에 새 업무 로직(카드사별 인증·발급 절차)을 반영하려고 서버를 재시작하면 그 시점에 연결돼 있던 모든 지점 기기의 연결이 끊기고 재연결될 때까지 거래가 안 되는 문제가 있었습니다.",
          "그래서 업무 로직을 관리 서버에 별도 모듈 형태로 등록해두는 화면을 만들고, 관리자가 그 업무를 반영할 때는 반영(적재)과 초기화 버튼을 순서대로 눌러 관리 서버가 core 서버에 알리도록 했습니다. core 서버는 이 신호를 받으면 등록된 모듈을 동적으로 읽어 들여 해당 업무 코드에 연결해두고, 필요한 보안 장비(HSM) 키까지 초기화해서 그 업무를 즉시 쓸 수 있는 상태로 만듭니다.",
          "이 과정 전체가 core 서버를 껐다 켜지 않고 실행 중인 상태에서 이뤄지기 때문에, 반영이 끝난 직후 들어오는 지점 기기 요청부터 바로 새 로직으로 처리되고 기존에 연결돼 있던 다른 지점 기기들의 연결은 전혀 끊기지 않습니다."
        ]
      },
      {
      ...
      },
      {
        ...
        detail: [
          "카드에 개인화 데이터를 기록하는 통신은 카드와 여러 차례 요청·응답을 주고받으며 진행되는데, 이 통신이 규격대로 잘 동작하는지 확인하려면 매번 그 요청 전문을 직접 만들어서 보내야 했습니다. 실물 카드와 프린터가 연결된 환경에서만 확인할 수 있었던 것도 테스트를 번거롭게 만드는 요인이었습니다.",
          "그래서 URL과 카드번호 등 몇 가지 값만 입력하면, 첫 요청을 보내고 그 응답을 보고 다음 요청을 자동으로 만들어 순서대로 보내는 방식으로 통신 과정 전체를 한 번에 돌려볼 수 있는 테스트 페이지를 만들었습니다.",
          "실물 카드가 없는 상태에서는 카드가 보내야 할 응답 값 자체를 알 수 없기 때문에, 통신 횟수에 맞춰 성공을 뜻하는 값으로 채워 넣어 통신이 끝까지 정상적으로 진행되는지만 확인하는 방식으로 만들었고, 매 단계에서 보낸 전문과 받은 전문을 화면의 표로 바로 확인할 수 있게 했습니다. 이 기능은 테스트 전용으로 만든 것이라 운영 환경에는 배포하지 않도록 코드에 별도로 표시해뒀습니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          "데이터베이스와 보안 장비 상태를 1분마다 점검하는 예약 작업이 있는데, 문제가 발견되면 그때마다 별도의 통신 채널을 새로 열어 관리자에게 알림을 보내도록 돼 있었습니다. 그런데 이 알림을 보내는 코드에서 통신에 쓴 자원을 정리하는 부분이 빠져 있었습니다. 평소에는 문제가 드러나지 않았지만, 데이터베이스나 보안 장비 장애가 길게 이어지면 1분마다 점검할 때마다 자원이 하나씩 새로 생기기만 하고 정리되지 않아 계속 쌓였고, 결국 서버 전체가 자원 부족으로 잠깐씩 멈추는 현상으로 이어졌습니다.",
          "로그와 서버 상태를 분석해 원인을 알림 전송 코드의 자원 미정리로 특정한 뒤, 알림을 다 보내고 나면 반드시 그 통신 자원을 정리하도록 수정해 문제를 해결했습니다. 실제로 은행 데이터를 중계하는 통신 경로는 이미 자원을 정상적으로 정리하고 있었고, 문제는 관리자 알림을 보내는 경로에 한정돼 있었습니다."
        ]
      },
      {
        ...
        detail: [
          "국제 카드 표준(EMV)에 따른 비밀번호 변경·잠금해제 절차는 카드에서 온 값으로 세션키를 만들고, 그 세션키로 카드가 보낸 인증값이 맞는지 확인한 뒤, 새 비밀번호를 암호화하고 마지막으로 위변조를 막는 확인값을 계산해 카드에 내려보낼 명령을 조립하는 순서로 진행됩니다. 이 절차 자체는 카드 브랜드가 달라도 큰 흐름이 같지만, 세부 파라미터나 코드값은 브랜드마다 조금씩 달랐습니다.",
          "그래서 세션키 생성, 인증값 검증, 확인값 계산처럼 공통되는 부분은 공용 로직으로 만들어두고, 브랜드별로 다른 부분(코드값, 데이터 길이 등)만 별도 클래스에서 채워 넣는 구조로 설계했습니다. 그 결과 8개가 넘는 카드 브랜드에 대응하면서도 브랜드마다 전체 로직을 새로 작성하지 않고 대응할 수 있었습니다."
        ]
      },
      {
        ...
        detail: [
          "업무 로직 하나를 처리하는 과정에서 보안 장비 세션 등 공유 자원을 함께 쓰기 때문에, 같은 업무 코드에 대한 요청이 동시에 들어오면 값이 뒤섞일 위험이 있었습니다. 그렇다고 서버 전체를 하나씩 순서대로 처리하게 만들면, 서로 무관한 업무 코드까지 불필요하게 대기하게 돼 성능이 떨어질 수 있었습니다.",
          "그래서 업무 코드별로 로직 인스턴스를 하나만 만들어 캐시해두고 재사용하되, 그 인스턴스가 요청을 처리하는 메서드 자체를 동기화 처리해서 같은 업무 코드에 대한 요청은 순서대로 하나씩만 처리되고, 서로 다른 업무 코드에 대한 요청은 각자의 인스턴스에서 동시에 처리될 수 있도록 만들었습니다."
        ]
      },
      {
        ...
        detail: [
          "EAI 서버로 전문을 보낼 때는 응답을 다 전송한 뒤 연결을 닫아 마무리하는 구조였는데, 처음에는 연결을 닫으라는 요청만 보내고(비동기 호출) 실제로 언제 닫히는지는 기다리지 않았습니다. 이 요청과 실제 종료 사이에는 시간차가 있어서, 아주 드물게 EAI 쪽이 확인(ACK) 전문을 다 받기 전에 연결이 먼저 끊겨버리는 경우가 있었고, 이런 경우 EAI 쪽에서는 응답을 받지 못한 것으로 판단해 타임아웃 처리를 했습니다.",
    ...
    challenges: [
      {
        ...
        detail: [
          "이 시스템은 이미 업무 로직을 업로드하면 서버 재시작 없이 바로 불러 쓸 수 있는 구조를 갖추고 있었는데, 로직 파일 자체는 서버의 파일 시스템에 저장해두는 방식이었습니다. 여러 서버 인스턴스가 같은 업무를 나눠 처리하다 보니, 파일을 인스턴스마다 개별적으로 맞춰둬야 했고, 복사가 누락되거나 버전이 어긋나면 인스턴스별로 다른 로직이 동작하는 위험이 있었습니다.",
          "그래서 업로드된 로직 파일 자체를 데이터베이스에 저장해두고, 실제로 그 로직을 불러올 때 데이터베이스에서 파일을 가져와 실행하도록 구조를 바꿨습니다. 이렇게 하면 서버 인스턴스가 몇 대든 항상 데이터베이스에 저장된 같은 원본을 보고 동작하게 됩니다."
        ]
      },
      {
        ...
        detail: [
          "데이터베이스가 끊기면 로직 실행에 필요한 정보 자체를 조회할 수 없어 서비스가 완전히 멈췄습니다.",
          "처음에는 특정 로직 하나와 보안 장비 슬롯 하나를 설정값으로 미리 지정해두고, 데이터베이스 조회가 실패하면 그 값을 그대로 쓰는 임시방편으로 문제를 넘겼습니다. 하지만 이 방식은 미리 지정해둔 로직 하나만 대응할 수 있어서, 실제로 운영 중인 다른 로직들은 데이터베이스 장애 시 여전히 멈췄습니다.",
          "그래서 평소 정상 동작 중에는 데이터베이스에서 조회한 로직 목록 전체를 파일로도 함께 저장해두고, 데이터베이스 조회가 실패하는 시점에는 이 파일을 읽어 마지막으로 정상 조회됐던 로직 목록 전체를 그대로 복원하도록 다시 설계했습니다. 이렇게 하면 데이터베이스 장애 중에도 이미 알려진 모든 로직이 계속 서비스될 수 있습니다."
        ]
      },
      {
        ...
        detail: [
          "여러 서버 인스턴스가 업무를 나눠 처리하는 구조라, 업무 상태를 인스턴스 간에 공유할 방법이 필요했습니다. 처음에는 Redis를 도입해 이중화 구성까지 갖춰 상태를 공유하도록 구현했는데, 실제 운영 규모와 보안 장비 세션의 특성을 고려했을 때 Redis를 운영하는 것 자체가 추가로 관리해야 할 부담이 크다는 걸 확인했습니다.",
          "그래서 이후에는 Redis를 걷어내고, 원래 쓰던 데이터베이스 기반의 단순한 상태 관리 구조로 되돌렸습니다. 화려한 기술보다 실제 운영 규모에 맞는 단순한 구조가 더 낫다는 걸 판단하고 실행에 옮긴 경험이었습니다."
        ]
      },
      {
        ...
        detail: [
          "업무 로직을 처리하고 그 결과를 곧바로 반환값으로 돌려주는 구조였는데, 여러 클라이언트의 요청이 동시에 들어오는 환경에서는 처리 순서와 반환 시점이 엇갈리면서 한 클라이언트가 다른 클라이언트의 결과를 받아가는 문제가 발생했습니다. 원인을 살펴보니 동시에 여러 스레드가 같은 처리 로직을 실행할 때, 결과를 돌려주는 시점과 요청·응답을 짝짓는 방식이 서로 맞물리지 않는 구조였습니다.",
          "그래서 처리 결과를 클라이언트별 식별값을 키로 하는 별도의 저장 공간에 담아두고, 각 클라이언트가 자신의 식별값으로 그 결과를 다시 조회해가는 방식으로 바꿔서, 동시에 여러 요청이 들어와도 결과가 서로 섞이지 않도록 만들었습니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          ...
          "여기에 더해 보안 장비 제조사가 배포하는 연동 라이브러리는 Windows용은 .dll, Linux용은 .so, Mac용은 .dylib 파일로 완전히 다르고, 설치 경로도 운영체제마다 관례가 달랐습니다. 심지어 같은 Windows 환경이어도 라이브러리 파일명이 처음 설정해둔 이름과 실제 설치된 파일명이 미묘하게 달라서(예: 64비트 전용 파일명이 따로 있는 경우) 그대로 실행하면 연결 자체가 실패하는 경우도 있었습니다.",
          "그래서 설정 파일에 운영체제별 경로를 전부 적어두고, 실행하는 환경에 맞는 줄만 활성화해서 쓰도록 정리했고, 실제 설치된 라이브러리 파일명을 직접 확인해 정확한 파일명으로 맞추는 과정을 거쳤습니다."
        ]
      },
      {
      ...
      },
      {
        ...
        detail: [
          "보안 장비에 새 키를 만들 때는 여러 속성값(민감 정보 여부, 사용 가능한 알고리즘 등)을 함께 넘겨야 하는데, 이 조합이 안 맞으면 보안 장비가 돌려주는 오류는 실패했다는 사실만 알려줄 뿐 정확히 어떤 속성이 문제인지는 알려주지 않았습니다. 그래서 속성값을 하나씩 주석 처리하거나 다시 넣어보면서 실제로 통과하는 조합을 직접 찾아나갔고, 그렇게도 확실하지 않은 부분은 보안 장비 제조사(Entrust)에 직접 문의해서 정확한 속성값을 확인했습니다.",
          "또한 장비 버전에 따라 필요한 속성값이 달라질 수 있다는 점도 함께 확인해, 버전이 다른 장비에서도 동일하게 동작하는지 점검했습니다."
        ]
      }
    ]
  },
  {
    ...
    challenges: [
      ...
      },
      {
        ...
        detail: [
          "실험용 포털이다 보니 개발자들이 실험용 서버를 띄워놓고 그대로 방치하는 일이 잦아 비용이 계속 누적됐습니다. AWS/Azure/구글 클라우드를 모두 지원해야 했으므로, 클라우드 공급자별 자원 종료 로직을 공통된 방식으로 설계하고 예약 작업이 프로젝트에 맞는 클라우드 로직을 자동으로 찾아 호출하도록 구성했습니다.",
          "구글 클라우드 쪽에서는 서버, 컨테이너 클러스터, 데이터베이스를 순서대로 점검해 켜져 있는 자원을 모두 종료시킵니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          "정해진 파일 이름 규칙으로 이미지를 한 번에 여러 개 선택해서 올리면, 서버가 각 파일 이름으로 대상 접수기관을 찾아 해당 기관에만 이미지를 반영하도록 구현했습니다. 파일 이름과 일치하는 접수기관이 없으면 그 건만 실패로 기록하고 나머지는 그대로 진행되도록 처리해, 한두 건이 잘못 올라와도 전체 업로드가 막히지 않습니다.",
          "처리가 끝나면 전체·성공·실패 건수와 실패 이유를 함께 알려줘서, 관리자가 어떤 파일이 왜 실패했는지 바로 확인할 수 있습니다."
        ]
      },
      {
        ...
        detail: [
          "저장공간·서비스가입·발급유형별 수량 등 7가지 통계마다 방식을 통일했습니다. 목록 조회 기능이 화면용 데이터를 페이지 단위로 주고, 같은 검색 조건을 받는 엑셀 다운로드 기능은 전체 건수를 한 번에 조회해서 엑셀로 만들어 응답합니다.",
          "통계 항목(열)을 미리 정의해두고 표 머리글을 자동으로 뽑아내게 했더니, 통계 종류가 늘어나도 매번 같은 구조의 기능만 추가하면 됐습니다."
        ]
      },
      {
        ...
        detail: [
          "증명서 종류가 늘어나면서 서로 관련 있는 증명서들을 이용자에게 각각 따로 보여주면 어떤 것들이 함께 필요한지 알기 어려웠습니다.",
          "그래서 관련 증명서 종류 여러 개를 하나의 패키지로 묶어서 관리할 수 있는 화면을 만들고, 실제 서비스에서 증명서 목록을 조회할 때는 개별 증명서뿐 아니라 패키지 단위로 묶인 항목도 함께 내려주도록 구현했습니다. 이렇게 하면 관련 증명서를 새로 추가할 때도 패키지 구성만 조정하면 되고, 이용자는 필요한 증명서들을 묶음으로 한눈에 확인할 수 있습니다."
        ]
      },
      {
        ...
        detail: [
          ...
          "그래서 관리자 화면에 캐시 종류(증명서/약관/제출처/공지사항/배너 등)와 적용할 환경을 선택해 확인 후 즉시 초기화하는 기능을 추가했습니다. 관리자가 이 기능을 실행하면 실제 서비스 서버에 캐시를 비우라는 요청이 전달되고, 그 순간부터는 최신 데이터를 다시 가져와 보여주게 됩니다. 재배포나 서버 재시작 없이도 수정 내용을 바로 반영할 수 있게 된 것입니다."
        ]
      }
    ]
  }
...
const personalProjects = [
    ...
    challenges: [
      {
        ...
        detail: [
          "하객이 직접 접수하는 화면에서는 이체를 완료했다고 누른 순간 서버에 접수 내용을 저장해야 하는데, 결혼식장 지하처럼 통신이 약한 곳에서는 이 요청이 서버까지 아예 도달하지 못하는 경우가 있었습니다. 이때 그냥 오류만 보여주면 하객이 접수를 포기하거나 중복으로 다시 시도해 혼란이 생길 수 있었습니다.",
          "그래서 서버가 응답은 했지만 값이 잘못돼 거절한 경우와, 응답 자체가 안 와서 재시도하면 될 수도 있는 진짜 네트워크 단절을 구분했습니다. 진짜 단절인 경우에는 접수 내용을 기기에 임시로 저장해두고, 하객에게는 평소와 똑같이 감사 화면을 보여줘서 접수가 끊겼다는 걸 느끼지 못하게 했습니다.",
          "그 뒤로는 브라우저가 다시 온라인 상태가 되는 시점과 일정 주기마다 저장해둔 접수 건들을 순서대로 서버에 다시 보내도록 했고, 재전송 중에 또 네트워크가 끊기면 남은 건들은 그대로 두고 다음 시도를 기다리게 했습니다.",
          "접수는 성공했는데 식권 발급만 실패한 경우에는, 접수 자체는 이미 저장됐으니 그 건은 큐에서 빼고 식권만 나중에 접수 장부에서 수동으로 처리할 수 있게 했습니다."
        ]
      },
      {
        ...
        detail: [
          "식권 한 장에는 대인·소인 인원이 함께 배정될 수 있는데, 배정된 인원이 반드시 한 번에 다 같이 입장하는 게 아니라 일부는 먼저 도착해 입장하고 나머지는 나중에 입장하는 경우도 있었습니다. 그래서 체크인을 '남은 인원을 조회하고 문제없으면 차감'하는 두 단계로 만들면, 검표대가 여러 곳이거나 같은 QR이 거의 동시에 스캔될 때 조회 시점엔 둘 다 인원이 남아있는 것으로 보여 배정된 인원보다 많은 사람이 들어가 버릴 수 있었습니다.",
          "그래서 조회와 차감을 분리하지 않고, '차감 후에도 잔여 인원이 요청 인원 이상이어야 한다'는 조건을 갱신 쿼리 자체에 포함시켜 검증과 차감을 하나의 db 요청으로 처리했습니다. 이렇게 하면 동시에 여러 요청이 들어와도 db가 순서를 정해주기 때문에 배정 인원을 넘는 요청부터는 자동으로 거부됩니다.",
          "인원이 모두 소진되면 식권 상태를 자동으로 바꿔 이후 스캔 시 바로 알 수 있게 했고, QR 유출이 의심되면 기존 QR을 무효화하고 새 QR을 발급할 수 있게 했습니다.",
          "또한 식권이 발급되는 시점과 실제 입장 가능 여부를 서로 다른 조건으로 뒀습니다 — 식권 자체는 접수(부조금 신고) 시점에 나오지만, 실제 입장은 그 접수 건의 입금이 확인된 뒤에만 가능하도록 한 번 더 걸러서, 아직 입금 확인이 안 된 접수 건으로 먼저 입장해버리는 상황을 막았습니다."
        ]
      },
      {
        ...
        detail: [
          "경조사비는 물건이나 서비스에 대한 대가가 아니라 순수한 축하·조의의 송금이라, 카드결제망(PG) 연동은 사업자 등록과 심사, 정산 주기, 수수료까지 감안하면 배보다 배꼽이 더 큰 방식이라고 판단했습니다. 가상계좌 발급도 마찬가지로 PG 계약이 필요해 같은 이유로 제외했습니다.",
          "대신 수취인(신랑측·신부측 등)의 실제 계좌로 하객이 바로 이체하게 하고, 이체 시 '예상 입금자명'을 함께 입력받아 저장해뒀습니다. 이 입력은 키오스크뿐 아니라 모바일 청첩장 페이지 자체에도 계좌 정보 바로 아래에 '마음 전하기' 폼으로 넣어서, 하객이 청첩장을 보다가 그 자리에서 이체하고 이체했다는 사실까지 바로 알릴 수 있게 했습니다.",
          "다만 이체하는 사람이 실제 계좌 명의자 이름 그대로 보내지 않는 경우가 흔해서, 하객이 남긴 '이체했다'는 응답값과 실제 계좌에 찍힌 입금 내역을 시스템이 자동으로 매칭하는 건 신뢰하기 어렵다고 판단했고, 그래서 예상 입금자명과 하객 이름을 화면에 나란히 보여주고 주최자가 자신의 뱅킹 앱과 대조해 직접 입금 확인 버튼을 누르는 방식으로 만들었습니다.",
          "완전 자동 매칭(오픈뱅킹 연동)은 사업자 등록 심사 등의 진입장벽이 있어 이번 범위에서는 제외하고, 대신 카카오페이·모임통장 링크를 QR에 함께 넣어 하객이 송금하기 편하게 만드는 선에서 개선했습니다."
        ]
      },
      {
        ...
        detail: [
          "실제로 결혼식장에 상시 놓인 키오스크 기기를 두려면 결혼식장(웨딩홀) 측과 정식으로 제휴를 맺어야 하고, 키오스크 전용 하드웨어(터치스크린 거치대 등)도 별도로 구매·설치해야 해서 시간과 비용이 많이 드는 일이었습니다.",
          "이런 물리적인 준비 없이도 바로 쓸 수 있는 방법이 필요해서, 노트북 한 대만 있으면 동작하는 완전히 별도의 가벼운 프로그램을 만들었습니다. 결혼식 당일 노트북에서 임시 서버로 띄우면 같은 와이파이에 있는 태블릿 여러 대가 접속해 접수할 수 있고, 행사가 끝나면 접수 내역을 엑셀로 추출한 뒤 그 프로그램은 그대로 폐기합니다."
        ]
      }
    ]
  },
  {
    ...
    challenges: [
      ...
      },
      {
        ...
        detail: [
          "여러 탭이나 여러 기기로 동시에 접속해 있을 때, 그중 하나만 닫혀도 전체 접속이 끊긴 것으로 착각해 온라인 상태를 꺼버리면 실제로는 다른 탭에서 계속 쓰고 있는데도 상대방에게는 오프라인으로 보이는 문제가 있었습니다. 그래서 접속을 관리하는 곳에서 사용자별로 지금 열려 있는 접속 수를 함께 추적하도록 해서, 그 수가 0이 될 때(정말 마지막 하나가 끊길 때)만 오프라인으로 전환되도록 만들었습니다.",
          "또한 마지막 접속이 끊기더라도, 그 사용자가 자리비움·방해금지처럼 직접 골라둔 상태였다면 오프라인으로 덮어쓰지 않고 그 상태 그대로 유지되도록 분리했습니다(온라인 상태였을 때만 오프라인으로 바뀝니다).",
          "다만 새로 접속을 시작하는 시점에는 이전에 어떤 상태를 골라뒀었는지와 무관하게 항상 온라인으로 초기화되도록 의도적으로 단순하게 만들었습니다 — 접속이 끊긴 뒤 다시 들어올 때마다 매번 온라인으로 시작하고, 자리비움·방해금지는 그때그때 다시 고르는 방식입니다."
        ]
      },
      {
        ...
        detail: [
          "채팅 내용은 개인정보라 암호화해서 저장했는데, 그러면 저장된 값이 암호문이라 일반적인 '~를 포함한 메시지 찾기' 방식의 검색이 통하지 않았습니다. 그렇다고 검색할 때마다 모든 메시지를 복호화해서 비교하는 건 성능도 떨어지고 암호화한 의미도 퇴색됩니다.",
          "그래서 메시지를 저장할 때 본문과는 별도로, 짧은 글자 단위로 쪼갠 뒤 되돌릴 수 없는 방식으로 변환한 검색용 값을 함께 저장해뒀습니다. 검색어가 들어오면 똑같은 방식으로 변환해서, 저장된 검색용 값과 일치하는지만 비교합니다. 이렇게 하면 원문 내용을 몰라도 '이 메시지에 이 검색어가 들어있는지' 정도는 판단할 수 있고, 한글처럼 띄어쓰기 없이 붙여 쓰는 경우에도 부분적으로 검색이 됩니다.",
          "암호화 처리를 담당하는 부분을 잘못 등록해서 하마터면 관련 없는 다른 데이터까지 전부 암호화될 뻔한 실수도 있었는데, 적용 범위를 딱 필요한 항목 하나로 좁혀서 해결했습니다."
        ]
      },
      {
        ...
        detail: [
          "이 채팅 프로그램은 사내에서만 쓰는 데스크톱 앱이라 유료 개발자 인증(공증) 절차 없이 만들어 배포했는데, macOS의 보안 기능(Gatekeeper)이 출처가 확인되지 않은 앱이라며 실행 자체를 막았습니다. 사용자 입장에서는 '손상되었기 때문에 열 수 없습니다'라는 오해하기 쉬운 오류 메시지만 보였습니다.",
          "원인은 인증서 서명·공증이 없는 앱이라는 점이었고, 사내용으로 공증까지 받기엔 과한 절차라고 판단해, 빌드 시 관련 보안 검사 옵션을 사내 배포 상황에 맞게 조정했습니다.",
          "그래도 이미 다운로드한 사용자의 macOS에는 이미 '검역' 표시가 남아있을 수 있어서, 터미널에서 간단한 명령어 한 줄로 그 표시를 지우면 정상 실행된다는 안내 문서를 함께 만들어 배포했습니다."
        ]
      }
    ]
  },
  {
    ...
    screenshots: [
      ...
      { src: "images/screenshots/baby-yeolmu-dashboard.png", caption: "홈 대시보드 — 임신 주차, 태아 성장 정보, 이번 주 할 일을 한눈에" },
      ...
      { src: "images/screenshots/baby-yeolmu-fourcut.png", caption: "인생네컷 — 레이아웃 선택부터 시작하는 5단계 제작 과정" },
      { src: "images/screenshots/baby-yeolmu-fourcut-deco.png", caption: "인생네컷 — 배경/텍스트/스티커를 조합해 완성한 콜라주" }
    ],
    challenges: [
      {
      ...
      },
      {
        ...
        detail: [
          "달력 페이지는 원래 그 달의 기록을 조회한 뒤, 기록마다 미디어 파일을 한 건씩 추가로 조회하고 있어서 기록이 30개면 조회가 31번 나가는 전형적인 N+1 문제가 있었습니다. 그래서 기록들을 한 번에 가져온 뒤, 그 기록들의 ID를 모아서 미디어 파일도 한 번에 통째로 조회하고 메모리에서 기록별로 다시 짝지어주는 방식으로 바꿔 조회를 31번에서 2번으로 줄였습니다.",
          "또한 게스트 인증을 확인하는 로직이 화면에 쓰이는 이미지·CSS·JS 파일 요청에까지 매번 데이터베이스에서 권한을 다시 조회하고 있어서, 페이지 하나를 열 때 권한 조회만 20~30번씩 나가고 있었습니다. 그래서 그런 정적 파일 요청은 권한 확인 로직 자체를 건너뛰도록 처리하고, 권한 조회 결과는 5분 동안 잠시 기억해뒀다가 재사용하도록 만들어서 페이지당 권한 조회를 1번 수준으로 줄였습니다."
        ]
      },
      {
        ...
        detail: [
          "초대 링크를 받는 사람마다 보여주고 싶은 범위가 다릅니다 — 사진만 살짝 보여주고 싶은 지인도 있고, 병원 기록까지 함께 챙겨야 하는 가족도 있습니다. 그래서 초대장을 만들 때 '사진 보기', '댓글 작성', '다운로드', '네컷 만들기', '사진 삭제', '달력', '성장/수유/수면', '예방접종', '진료기록', '초대 관리', '문서 보관함'처럼 항목을 세분화해서 원하는 것만 체크해 담을 수 있게 만들었습니다.",
          "예를 들어 '사진 보기'만 체크해서 링크를 만들면, 그 링크로 들어온 사람 화면에는 정말 사진 메뉴 하나만 보이고 나머지 메뉴(달력, 병원 기록 등)는 아예 나타나지 않습니다.",
          "그리고 이건 화면에서만 숨긴 게 아니라, 그 사람이 주소를 직접 입력해서 다른 화면에 들어가려고 해도 서버가 그 권한이 없다는 걸 확인하고 막습니다. 이렇게 하면 사진을 외부에 유출할 걱정 없이, 정말 보여주고 싶은 사람에게 딱 보여주고 싶은 것만 공유할 수 있습니다."
        ]
      }
    ]
  },
    ...
    challenges: [
      {
        ...
        detail: [
          "카드 프린터는 컬러 잉크 리본과 흑백 리본을 따로 인쇄 헤드에 걸어 찍는 경우가 많아서, 같은 배치라도 컬러용과 흑백용 이미지를 각각 만들어야 했습니다. 그런데 글자·사진·바코드 같은 요소 하나하나에 '흑백일 때는 이렇게, 컬러일 때는 이렇게' 그리는 로직을 따로 만들면 요소가 늘어날 때마다 관리 부담이 두 배가 됩니다.",
          "그래서 요소를 그리는 코드는 하나로 통일하고, 그 그림을 최종적으로 담는 이미지판을 흑백 전용/컬러 전용으로 미리 다르게 준비해서, 같은 그리기 코드를 두 번 실행하는 것만으로 자동으로 흑백·컬러 결과가 나뉘도록 만들었습니다. 이렇게 하면 앞면 컬러·앞면 흑백·뒷면 컬러·뒷면 흑백, 이 4가지 이미지가 요소별 특수 처리 없이 한 번의 처리 과정에서 모두 생성됩니다."
        ]
      },
      {
      ...
      },
      {
        ...
        detail: [
          "카드가 프린터에 실제로 들어가는 방향은 화면에서 보기 편한 방향과 정반대인 경우가 있었습니다. 그래서 배치 데이터 자체는 그대로 두고, 실제 인쇄용 이미지를 만드는 시점에만 전체를 180도 회전시켜서 내보내도록 만들었습니다.",
          "반면 화면에 띄우는 미리보기 이미지는 인쇄용과는 다른 90도 회전이 필요해서, 인쇄용 처리와 미리보기용 처리를 서로 다른 회전값으로 나눠서 구현했습니다.",
          "도서관에서 쓰는 카드처럼 예외적인 카드 종류는 이 회전 처리 자체를 건너뛰도록 조건을 나눠서, 카드 종류마다 실제 프린터와 화면에 맞는 방향으로 정확히 나가도록 만들었습니다."
        ]
      },
      {
        ...
        detail: [
          "특정 카드 종류(도서관용)는 원래 다른 카드들과 다른 방향으로 인쇄되도록 회전 처리 자체를 건너뛰고 있었는데, 그 카드의 디자인을 가로에서 세로로 바꾸면서 다른 카드들과 똑같이 회전을 걸어야 했습니다. 문제는 이 카드의 좌표들이 원래 회전이 안 걸리는 걸 전제로 잡혀 있었기 때문에, 회전을 그대로 적용하면 요소들이 캔버스 밖으로 밀려나거나 위치가 어긋났습니다.",
          "그래서 회전을 걸기 전에 전체 요소 위치에 일정한 보정값을 더해주는 함수를 만들었는데, 이 보정값을 무조건 세로 방향에 더하면 안 됐습니다 — 이미 자기 자신이 90도나 270도로 회전된 개별 요소는 회전 때문에 가로·세로 개념이 서로 뒤바뀌어 있어서, 그런 요소에는 같은 보정값을 가로 방향에 더해야 실제로 원하는 방향으로 이동했습니다. 그래서 요소마다 자기 회전 각도를 확인해서 보정값을 넣을 축을 다르게 선택하도록 처리했습니다.",
          "여백(margin) 값은 방향이 바뀐다고 자동으로 재계산해주는 로직이 없어서, 레이아웃을 다시 짤 때 좌표와 함께 사람이 손으로 다시 잡아야 했습니다. 보정값 자체도 처음엔 감으로 넣었다가 실제 인쇄 결과가 안 맞아서, 현장 요청을 받고 다시 조정한 이력이 있습니다."
        ]
      }
    ]
  }
